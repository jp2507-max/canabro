        ...
        FOR j IN 0..jsonb_array_length(created_records)-1 LOOP
          ...
          WITH cols AS (
            SELECT 
              string_agg(
                CASE 
                  WHEN key = 'strain' THEN '"strain"'  -- Ensure "strain" is quoted
                  ELSE format('%I', key) 
                END, ', '
              ) AS cols, -- Always quote column names
              string_agg(
                CASE 
                  ...
                  WHEN key = 'strain' THEN format('($1->>''%s'')::text', key) -- Special handling for strain value
                  WHEN key = 'strain_id' THEN format('($1->>''%s'')::text', key) -- Corrected: strain_id is text in plants table
              ...
              string_agg(
                CASE 
                  WHEN key = 'strain' THEN '"strain" = EXCLUDED."strain"'  -- Ensure "strain" is quoted
                  ELSE format('%I = EXCLUDED.%I', key, key) 
                END, ', '
              ) AS updates -- Always quote column names for ON CONFLICT
            ...
            SELECT 
              string_agg(
                CASE 
                  ...
                  WHEN key = 'strain' THEN format('"strain" = ($1->>''%s'')::text', key, key) -- Ensure "strain" is quoted on LHS
                  WHEN key = 'strain_id' THEN format('%I = ($1->>''%s'')::text', key, key) -- Corrected: strain_id is text in plants table
...
EXCEPTION
  WHEN others THEN
    -- Log error information without using PG_EXCEPTION_CONTEXT
    RAISE NOTICE 'Error in sync_push: %, SQLSTATE: %', 
      SQLERRM, 
      SQLSTATE;
