...
interface TaskCardWithSelectionProps {
  task: PlantTask;
  plantName: string;
  plantImage?: string;
...
const TaskCardWithSelection = memo<TaskCardWithSelectionProps>(({ 
  task, 
  plantName, 
  plantImage, 
  onPress, 
  ...
  // Card animation hook - modified for selection mode
  ...
  const selectionAnimatedStyle = useAnimatedStyle(() => {
    const backgroundColor = isSelected 
      ...
      : '#F3F4F6'; // direct color value for unselected state (neutral-100)
    return {
  ...
  const selectionHighlightStyle = useAnimatedStyle(() => ({
    borderColor: withSpring(isSelected ? TASK_COLORS_DIRECT[taskTypeKey] : 'transparent'),
  ...
  // Swipe gesture handler using modern Gesture API - disabled in selection mode
    ...
    <GestureDetector gesture={panGesture}>
      <Animated.View style={[swipeAnimatedStyle]} className="mb-3">
        <Animated.View style={[completionAnimatedStyle]}>
          <AnimatedPressable
            style={[cardAnimatedStyle, selectionHighlightStyle]}
            {...handlers}
            className={`
          ...
          >
            <ThemedView variant="card" className="p-4 bg-transparent border-0">
              <View className="flex-row items-center space-x-3">
                ...
                {/* Completion Checkbox (shown in normal mode) */}
                {!showSelection && (
...
TaskCardWithSelection.displayName = 'TaskCardWithSelection';

export default TaskCardWithSelection;