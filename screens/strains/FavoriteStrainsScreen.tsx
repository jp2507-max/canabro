...
function useFavoriteStrains(
  favoriteData: {
    ...
    favoriteStrainIdPairs: Array<{uuid: string; objectId?: string}>;
  ...
  return useQuery<Strain[]>({
    queryKey: ['userFavoriteStrains', favoriteStrainIds, searchQuery, activeFilters, selectedStrainType],
    queryFn: async () => {
      ...
      for (let i = 0; i < batches; i++) {
        ...
        await Promise.all(batch.map(async ({ uuid, objectId }) => {
          try {
            // Always prefer the MongoDB ObjectId for API calls if available
            const apiId = objectId && isObjectId(objectId) 
...
export function FavoriteStrainsScreen() {
  ...
  const handleToggleFavorite = useCallback(async (strain: Strain) => {
  ...
  if (!userId)
    return (
      <View className="flex-1 items-center justify-center bg-white dark:bg-black">
        ...
        <ThemedText className="text-base mb-4 text-center">Please log in to view favorite strains.</ThemedText>
      </View>
    );
