...
import { View, ScrollView, FlatList, ActivityIndicator, RefreshControl, TextInput, TouchableOpacity, Pressable, Dimensions } from 'react-native';
...
import StrainFilterModal from '@/components/strains/StrainFilterModal';
...
const StrainCard = memo(({ item, onPress, isDarkMode, isFavorite, onToggleFavorite }: { 
  item: any; 
  onPress?: () => void; 
  isDarkMode?: boolean; 
  isFavorite?: boolean; 
  ...
  const determineStrainType = (): 'sativa' | 'indica' | 'hybrid' => {
    ...
    if (item.genetics) {
      ...
      if (lowerGenetics.includes('indica/sativa') || 
          lowerGenetics.includes('sativa/indica') ||
          lowerGenetics.includes('hybrid')) return 'hybrid';
  ...
  const safeItem = {
    ...
    description: Array.isArray(item.description) ? item.description[0] : (item.description || 'No description available.'),
    growDifficulty: item.growDifficulty || null,
    effects: Array.isArray(item.effects) ? item.effects : (Array.isArray(item.effect) ? item.effect : []),
    flavors: Array.isArray(item.flavors) ? item.flavors : 
             (Array.isArray(item.smellAndFlavour) ? item.smellAndFlavour : []),
  };
    ...
    >
      <View style={{ overflow: 'hidden', borderRadius: 24 }}>
        <ExpoImage
          ...
          style={{ width: width - 32, height: 210, borderTopLeftRadius: 24, borderTopRightRadius: 24 }}
          contentFit="cover"
        ...
        <View style={{ 
          ...
          <MaterialCommunityIcons 
            ...
            style={{ marginRight: 4 }} 
          />
          <ThemedText className="text-xs font-bold" style={{ color: '#fff', textShadowColor: '#000', textShadowRadius: 4 }}>
            {safeItem.type.charAt(0).toUpperCase() + safeItem.type.slice(1)}
          </ThemedText>
        </View>
        <TouchableOpacity
          style={{ position: 'absolute', top: 18, right: 18, backgroundColor: safeIsDarkMode ? '#27272a' : '#fff', borderRadius: 16, padding: 7, elevation: 4 }}
          accessibilityRole="button"
          accessibilityLabel={safeIsFavorite ? 'Remove from favorites' : 'Add to favorites'}
          activeOpacity={0.8}
          onPress={handleToggleFavorite}
        >
          <Ionicons name={safeIsFavorite ? 'heart' : 'heart-outline'} size={22} color={safeIsFavorite ? '#f43f5e' : '#a1a1aa'} style={{ transform: [{ scale: safeIsFavorite ? 1.15 : 1 }] }} />
        </TouchableOpacity>
      </View>
      <View className="px-6 py-5">
        <ThemedText className="text-2xl font-extrabold mb-1 capitalize" style={{ letterSpacing: 0.2 }}>{safeItem.name}</ThemedText>
        <View className="flex-row items-center mb-2">
          {safeItem.growDifficulty && (
            <View style={{ backgroundColor: '#fde68a', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3, marginRight: 8 }}>
              <ThemedText className="text-xs font-semibold text-yellow-800">{safeItem.growDifficulty}</ThemedText>
            </View>
          )}
        </View>
        <ThemedText numberOfLines={2} className="mb-2 text-base text-neutral-600 dark:text-neutral-400">{safeItem.description}</ThemedText>
        <View className="flex-row flex-wrap mb-2">
          {safeItem.effects && safeItem.effects.slice(0, 2).map((effect: string, i: number) => (
            <EffectTag key={`${safeItem.id}-effect-${i}`} effect={effect as StrainEffectType} emoji />
          ))}
          {safeItem.effects && safeItem.effects.length > 2 && (
            <View style={{ backgroundColor: '#f3f4f6', borderRadius: 8, paddingHorizontal: 6, marginLeft: 4, justifyContent: 'center' }}>
              <ThemedText className="text-xs text-neutral-600 dark:text-neutral-400">+{safeItem.effects.length - 2} more</ThemedText>
            </View>
          )}
        </View>
        <View className="flex-row flex-wrap mb-2">
          {safeItem.flavors && safeItem.flavors.slice(0, 2).map((flavor: string, i: number) => (
            <FlavorTag key={`${safeItem.id}-flavor-${i}`} flavor={flavor as StrainFlavorType} emoji />
          ))}
          {safeItem.flavors && safeItem.flavors.length > 2 && (
            <View style={{ backgroundColor: '#f3f4f6', borderRadius: 8, paddingHorizontal: 6, marginLeft: 4, justifyContent: 'center' }}>
              <ThemedText className="text-xs text-neutral-600 dark:text-neutral-400">+{safeItem.flavors.length - 2} more</ThemedText>
            </View>
          )}
        </View>
      </View>
...
const CategoryChips = memo(({ selected = 'sativa', onSelect = () => {}, isDarkMode = false }: { 
  ...
  return (
    <ScrollView horizontal showsHorizontalScrollIndicator={false} className="mb-3" contentContainerStyle={{ paddingHorizontal: 8, gap: 12 }}>
      {safeCategories.map((cat) => {
        ...
        return (
          <TouchableOpacity
            ...
            style={{
              backgroundColor: selected === cat.id ? cat.color : isDarkMode ? '#27272a' : '#f3f4f6',
              borderRadius: 18,
              paddingVertical: 8,
          ...
          >
            <MaterialCommunityIcons name={cat.icon as any} size={20} color={selected === cat.id ? '#fff' : cat.color} style={{ marginRight: 7 }} />
            <ThemedText className="font-semibold text-base" style={{ color: selected === cat.id ? '#fff' : isDarkMode ? '#e5e7eb' : '#374151' }}>{cat.emoji || ''} {cat.name}</ThemedText>
          </TouchableOpacity>
        );
      })}
    </ScrollView>
...
const StrainsView: React.FC<Partial<StrainsViewProps>> = ({
  ...
  isFetching = false,
  error = null,
  activeFilters = { species: null, effects: [], flavors: [], minThc: null, maxThc: null, minCbd: null, maxCbd: null },
  handleApplyFilters = () => {},
  handleRefresh = () => {},
  ...
  if (error)
    return (
      <View className="flex-1 items-center justify-center bg-white dark:bg-black px-6">
        <ThemedText className="text-xl font-bold mb-2 text-center">Failed to load strains</ThemedText>
        <ThemedText className="text-base mb-4 text-center">{error.message || 'An error occurred.'}</ThemedText>
        <TouchableOpacity
  ...
  return (
    <SafeAreaView className="flex-1 bg-white dark:bg-black">
      ...
      <View className="px-4 pt-2">
        <CategoryChips selected={selectedStrainType} onSelect={setSelectedStrainType} isDarkMode={isDarkMode} />
        <View className="flex-row items-center mt-2 mb-3">
          <View className="flex-1 relative">
            <TextInput
              className="flex-1 px-4 py-3 rounded-xl bg-neutral-100 dark:bg-neutral-800 text-base text-neutral-800 dark:text-neutral-100"
            ...
            {isFetching && searchQuery ? (
              ...
              >
                <Ionicons name="close-circle" size={20} color={isDarkMode ? '#a1a1aa' : '#6b7280'} />
              </TouchableOpacity>
            ) : null}
          </View>
          <TouchableOpacity
            className="ml-2 p-2 rounded-full bg-neutral-200 dark:bg-neutral-700"
            accessibilityRole="button"
