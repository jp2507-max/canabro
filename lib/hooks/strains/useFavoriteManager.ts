...
} from '../../services/favorite-strain-service';
...
export function useFavoriteManager(): UseFavoriteManagerResult {
  ...
  const favoritesSet = useMemo(() => new Set(favoriteStrainIds), [favoriteStrainIds]);

  /**
   * Check if a strain is in the user's favorites - O(1) lookup with Set
   */
  const isFavorite = useCallback(
    (strainId: string): boolean => {
  ...
  const fetchFavorites = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        const idPairs: FavoriteStrainIdData[] =
          data?.map((item) => ({
            uuid: item.strain_id,
