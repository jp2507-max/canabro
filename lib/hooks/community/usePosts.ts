...
export function usePosts({ userId, limit = 10, offset = 0 }: UsePostsParams = {}) {
  return useQuery({
    ...
    queryFn: async (): Promise<PostData[]> => {
      ...
      // Use the existing RPC function that includes like status
      const { data, error } = await supabase.rpc('get_posts_with_like_status', {
...
export function useInfinitePosts({ userId }: { userId?: string }) {
