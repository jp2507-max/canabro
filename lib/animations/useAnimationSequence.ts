...
export function useAnimationSequence() {
  ...
  const runSequence = useCallback((config: ParallelAnimationConfig) => {
    ...
    const animationSteps = steps.map((step, index) => {
      const baseAnimation = step.type === 'spring' 
      ...
      return step.delay 
        ? withDelay(step.delay, withStepCallback)
        : withStepCallback;
    });
    ...
    if (onComplete && steps.length > 0) {
      ...
      animationSteps[lastStepIndex] = (() => {        const baseAnimation = lastStep.type === 'spring'
  ...
  const runStagger = useCallback((config: StaggerConfig) => {
    ...
    sharedValues.forEach((sharedValue, index) => {
      const delay = index * staggerDelay;        const animationFn = type === 'spring'
        ? withSpring(toValue, animConfig || SPRING_CONFIGS.smooth, () => {
            runOnJS(() => {
              completedCount++;
  ...
  const runParallel = useCallback((animations: Array<{
    ...
    animations.forEach(({ sharedValue, toValue, config: animConfig, type, delay }) => {
      const animationFn = type === 'spring'        ? withSpring(toValue, animConfig || SPRING_CONFIGS.smooth, () => {
            runOnJS(() => {
              completedCount++;
              if (completedCount === totalAnimations) {
                // Clean up tracking - must run on JS thread
...
export const SEQUENCE_PRESETS = {
  ...
  slideUpFadeIn: (
    ...
    parallel: [
      { sharedValue: translateY, toValue: 0, type: 'spring' as const, config: SPRING_CONFIGS.smooth },
