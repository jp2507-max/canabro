...
export function useAdvancedGesture(config: UseAdvancedGestureConfig = {}) {
  ...
  const panGesture = Gesture.Pan()
    ...
    .maxPointers(enableMultitouch ? maxPointers : 1)    .onStart(() => {
    ...
    .onEnd((event) => {
      ...
      if (Math.abs(event.velocityX) > momentumThreshold || Math.abs(event.velocityY) > momentumThreshold) {
  ...
  const pinchGesture = enablePinch ? Gesture.Pinch()
    .onStart(() => {
  ...
  const rotationGesture = enableRotation ? Gesture.Rotation()
    .onStart(() => {
