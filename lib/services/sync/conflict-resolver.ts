      ...
      try {
        ...
        if (existingIds.size > 0) {
          ...
          const stillCreated = result.created.filter(
            (record: { id: string }) => !existingIds.has(record.id)
          );
          const movedToUpdated = result.created.filter((record: { id: string }) =>
 ...
 */
async function resolvePlantConflicts(updatedPlants: any[], collection: any): Promise<void> {
 ...
 */
async function resolveEntryConflicts(updatedEntries: any[], collection: any): Promise<void> {
 ...
 */
async function resolveTaskConflicts(updatedTasks: any[], collection: any): Promise<void> {
 ...
 */
async function resolveJournalConflicts(updatedJournals: any[], collection: any): Promise<void> {
