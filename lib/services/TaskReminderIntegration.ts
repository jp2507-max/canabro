...
export class TaskReminderIntegration {
  ...
  private async cancelNotificationsForGrowthStage(
    ...
    try {
      // Get tasks for the specific growth stage that are no longer relevant
      const tasks = await database.collections
        .get<PlantTask>('plant_tasks')
        .query(
          Q.where('plant_id', plantId),
          Q.where('status', 'pending'),
          // Note: We would need to add growth_stage field to PlantTask model
  ...
  private async scheduleNotificationsForNextRecurringTask(completedTask: PlantTask): Promise<void> {
    try {
      ...
      const nextTask = await database.collections
        .get<PlantTask>('plant_tasks')
        .query(
