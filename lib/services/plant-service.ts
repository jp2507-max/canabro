...
export class PlantService extends BaseService {
  ...
  async getPlants(user_id: string): Promise<ApiResponse<Plant[]>> {
    try {
      ...
      return this.wrapResponse<Plant[]>((data || []).map(plant => this.adaptPlantFromDB(plant)), null);
    } catch (error) {
   ...
   * Creates a new plant
   */
  async createPlant(plant: Omit<Plant, 'id' | 'created_at' | 'updated_at'>): Promise<ApiResponse<Plant>> {
    try {
...
export const adaptPlantToDB = (plant: Partial<Plant>): Record<string, any> => plantService['adaptPlantToDB'](plant);
...
export const createPlant = async (plant: Omit<Plant, 'id' | 'created_at' | 'updated_at'>): Promise<Plant | null> => {
  const response = await plantService.createPlant(plant);
  return response.data;
};

export const updatePlant = async (plantId: string, updates: Partial<Plant>): Promise<Plant | null> => {
