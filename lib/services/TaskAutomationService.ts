...
// Local type safety helpers for difficulty metadata
// Some upstream data sources may expose either snake_case or camelCase fields.
export interface StrainData {
  id?: string;
  name?: string;
  // Optional difficulty fields in different naming conventions
  grow_difficulty?: string | null;
  growDifficulty?: string | null;
}

export interface PlantWithDifficulty extends Plant {
  // Optional difficulty metadata possibly denormalized on plant
  growDifficulty?: string | null;
  strainDifficulty?: string | null;
}
...
export class TaskAutomationService {
      ...
      await database.write(async () => {
        ...
        for (const taskType of stageConfig.recommendedTasks) {
          ...
          while (taskDate <= endDate) {
              ...
              const linkedStrain: StrainData | undefined = plant.strainId ? getStrainById(plant.strainId) : undefined;
              const difficulty: DifficultyLevel = normalizeDifficultyString(
                linkedStrain?.grow_difficulty ?? linkedStrain?.growDifficulty ??
                plant.growDifficulty ?? plant.strainDifficulty
              );
              ...
              newTask.weekNumber = Math.floor((taskDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24 * 7)) + 1;
              // Persist minimal strain metadata to support downstream guidance/notifications
              const metadata: PlantTask['strainMetadata'] = {
                // Only set difficulty when known and compatible with PlantTask schema
                difficulty: difficulty !== 'unknown' ? difficulty : undefined,
                strainId: plant.strainId ?? undefined,
              };
              newTask.strainMetadata = metadata;
            });
  ...
  /**
   * Compose base description with genetics/difficulty guidance snippet
   * to produce the final task description. Mirrors earlier logic to
   * keep behavior identical while avoiding duplication.
   */
  private static augmentTaskDescriptionWithGuidance(
    taskType: TaskType,
    growthStage: GrowthStage,
    strainType: StrainType,
    difficulty: DifficultyLevel
  ): string {
    const baseDescription = this.generateTaskDescription(taskType, growthStage);
    const guidance = buildTaskGuidanceSnippet({
      taskType,
      growthStage,
      strainType,
    ...
    return guidance ? `${baseDescription} ${guidance}` : baseDescription;
  }
      ...
      await database.write(async () => {
        ...
        while (taskDate <= finalEndDate) {
          const task = await database.get<PlantTask>('plant_tasks').create((newTask: PlantTask) => {
            ...
            newTask.description = this.augmentTaskDescriptionWithGuidance(
              taskType,
